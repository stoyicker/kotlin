...
class IdeReplExecutionTest : PlatformTestCase() {
    ...
    private fun checkOutput(expectedOutput: String) {
        val output =
            getReplOutput(textOnTimeOut = { "Only ${consoleRunner.commandHistory.processedEntriesCount} commands were processed" }) {
    ...
    fun testRunPossibility() {
        ...
        val hasErrors =
            { x: String -> x.contains("Process finished with exit code 1") || x.contains("Exception in") || x.contains("Error") }
